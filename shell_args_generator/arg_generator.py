...
class ArgGenerator:
    def __init__(self, parser_config: str, builder_config: str, input_path: str, output_dir: str):
    ...
    def bash_generate(self):
        ...
        self._bashBuilder.generate_bash_to_file(self._output_dir + "/startup.sh")

    def java_generate(self):
        if not os.path.isdir(self._output_dir):
            os.mkdir(self._output_dir)
        self._javaBuilder.generate_java_to_file(self._output_dir + "/CommandLineManager.java")

    def man_generate(self):
        if not os.path.isdir(self._output_dir):
            os.mkdir(self._output_dir)
        self._manBuilder.generate_man_to_file(self._output_dir + "/startup.8")
